...
# Pulling tweets
tweets_Dominik <- search_tweets("#CovidVaccine", n = 10000, lang = "en", include_rts = FALSE)
...
#Cleaning
clean_tweet_Dominik <- tweets_Dominik

url_regex <- "http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+"
clean_tweet_Dominik$text <- str_remove_all(clean_tweet_Dominik$text, url_regex) #remove url
clean_tweet_Dominik$text <- gsub("&amp", "", clean_tweet_Dominik$text) #remove html entity
clean_tweet_Dominik$text <- gsub("(RT|via)((?:\\b\\W*@\\w+)+)", "", clean_tweet_Dominik$text) #remove rt via
clean_tweet_Dominik$text <- gsub("@\\w+", "", clean_tweet_Dominik$text) #remove mentions
#clean_tweet$text <- str_replace_all(clean_tweet$text,"#[a-z,A-Z]*","") #remove hashtags
clean_tweet_Dominik$text <- gsub("[^[:alnum:]///' ]", " ", clean_tweet_Dominik$text)     #keep only alpha numeric 

print(clean_tweet_Dominik$text[1:30])

save(clean_tweet_Dominik, file = "clean_tweet_Dominik.RData")
...
tweets_short_Dominik <- clean_tweet_Dominik %>% select(user_id, created_at, status_id, text)

str(tweets_short_Dominik)
